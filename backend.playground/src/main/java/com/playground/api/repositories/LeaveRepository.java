...
public interface LeaveRepository extends JpaRepository<Leave, Long>{
	...
	@Query("select l from Leave l where l.employee.user.username=?1 AND l.status=?2 AND l.recordStatus=?3")
	List<Leave> getLeavesByEmployeeUsername(String username, LeaveEnum status, RecordStatus recordStatus);
