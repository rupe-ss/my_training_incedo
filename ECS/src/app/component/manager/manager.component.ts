...
export class ManagerComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.managerService
			.getEmployeeWithoutAccess(localStorage.getItem('token'))
			.subscribe({
				next: (data) => {
		...
		this.managerService
			.fetchLeavesPending(localStorage.getItem('token'))
			.subscribe({
				next: (data) => {
					this.leaves = data;
				},
		...
		this.managerService
			.getAllEmployees(localStorage.getItem('token'))
			.subscribe({
				next: (data) => {
					this.employees = data;
				},
	...
	grantAccess(email: string) {
		this.managerService
			.grantAccess(email, localStorage.getItem('token'))
			.subscribe({
				next: (data) => {
					this.employeeNoAccess = this.employeeNoAccess.filter(
						(e) => e.email !== email
					);
				},
			});
